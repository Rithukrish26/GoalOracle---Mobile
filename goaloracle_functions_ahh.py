...
st.markdown("<h3 style='text-align:center;color:#003366;'>Team A — Inputs</h3>", unsafe_allow_html=True)
ta_goals = st.number_input("Goals Scored (λ)", 0.0, value=1.2, step=0.1, key="ta_goals")
ta_conceded = st.number_input("Goals Conceded", 0.0, value=1.0, step=0.1, key="ta_conceded")
ta_sot = st.number_input("Shots on Target", 0.0, value=3.0, step=0.1, key="ta_sot")
ta_chances = st.number_input("Chances Created", 0.0, value=5.0, step=0.1, key="ta_chances")
ta_poss = st.number_input("Possession (%)", 0.0, 100.0, value=52.0, step=0.1, key="ta_poss")
ta_pass = st.number_input("Pass Completion (%)", 0.0, 100.0, value=82.0, step=0.1, key="ta_pass")

st.markdown("<h3 style='text-align:center;color:#003366;'>Team B — Inputs</h3>", unsafe_allow_html=True)
tb_goals = st.number_input("Goals Scored (λ)", 0.0, value=1.0, step=0.1, key="tb_goals")
tb_conceded = st.number_input("Goals Conceded", 0.0, value=1.1, step=0.1, key="tb_conceded")
tb_sot = st.number_input("Shots on Target", 0.0, value=2.7, step=0.1, key="tb_sot")
tb_chances = st.number_input("Chances Created", 0.0, value=4.0, step=0.1, key="tb_chances")
tb_poss = st.number_input("Possession (%)", 0.0, 100.0, value=48.0, step=0.1, key="tb_poss")
tb_pass = st.number_input("Pass Completion (%)", 0.0, 100.0, value=79.0, step=0.1, key="tb_pass")
...
if predict:
    lambda_a, lambda_b = ta_goals, tb_goals
